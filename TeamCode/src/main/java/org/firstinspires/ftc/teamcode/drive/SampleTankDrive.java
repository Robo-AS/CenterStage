...
public class SampleTankDrive extends TankDrive {
    ...
    public List<Double> getWheelPositions() {
        ...
        for (DcMotorEx leftMotor : leftMotors) {
            leftSum += encoderTicksToMM(leftMotor.getCurrentPosition());
        }
        for (DcMotorEx rightMotor : rightMotors) {
            rightSum += encoderTicksToMM(rightMotor.getCurrentPosition());
        }
    ...
    public List<Double> getWheelVelocities() {
        double leftSum = 0, rightSum = 0;
        for (DcMotorEx leftMotor : leftMotors) {
            leftSum += encoderTicksToMM(leftMotor.getVelocity());
        }
        for (DcMotorEx rightMotor : rightMotors) {
            rightSum += encoderTicksToMM(rightMotor.getVelocity());
        }
        return Arrays.asList(leftSum / leftMotors.size(), rightSum / rightMotors.size());
    }
